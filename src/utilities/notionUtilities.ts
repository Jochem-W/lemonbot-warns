...
export class NotionUtilities {
    ...
    public static async parseBlockObjects(blocks: BlockObjectResponse[] | AsyncGenerator<BlockObjectResponse>): Promise<ParseBlockObjectsResult> {
        ...
        if (blocks instanceof Array) {
            for (const block of blocks) {
                result = this.parseBlockObject(block, result)
            }
        } else {
    ...
    public static generateHyperlink(file: FileBlockObjectFileResponse, defaultCaption: string): string {
        let caption = file.caption.map(text => this.richTextToString(text)).join("")
        if (!caption) {
    ...
    private static parseBlockObject(block: BlockObjectResponse,
        ...
        switch (block.type) {
            case "paragraph":
                lastField.value += `${block.paragraph.rich_text.map(text => this.richTextToString(text)).join("")}\n`
                break
            case "heading_1":
                lastEmbed.addFields([{
                    name: block.heading_1.rich_text.map(text => this.richTextToString(text)).join(""),
                    value: " ",
                }])
                break
            case "heading_2":
                lastEmbed.addFields([{
                    name: block.heading_2.rich_text.map(text => this.richTextToString(text)).join(""),
                    value: " ",
                }])
                break
            case "heading_3":
                lastEmbed.addFields([{
                    name: block.heading_3.rich_text.map(text => this.richTextToString(text)).join(""),
                    value: " ",
                }])
                break
            case "bulleted_list_item":
                lastField.value +=
                    `• ${block.bulleted_list_item.rich_text.map(text => this.richTextToString(text)).join("")}\n`
                break
            case "numbered_list_item":
                lastField.value +=
                    `${result.currentListNumber}. ${block.numbered_list_item.rich_text.map(text => this.richTextToString(
                        text)).join("")}\n`
                result.currentListNumber++
                break
            case "quote":
                lastField.value += `> ${block.quote.rich_text.map(text => this.richTextToString(text)).join("")}\n`
                break
            case "to_do":
                lastField.value +=
                    `${block.to_do.checked ?
                        "✅" :
                        "🟩"} ${block.to_do.rich_text.map(text => this.richTextToString(text))
                        .join("")}\n`
                break
            case "toggle":
                lastField.value += `${block.toggle.rich_text.map(text => this.richTextToString(text)).join("")}\n`
                break
            case "equation":
            ...
            case "code":
                lastField.value +=
                    `${codeBlock(block.code.rich_text.map(text => this.richTextToString(text)).join(""),
                        block.code.language)}\n`
                break
            case "callout": {
                let icon
                ...
                lastField.value +=
                    `${codeBlock(`${icon} ${block.callout.rich_text.map(text => this.richTextToString(text))
                        .join("")}`)}\n`
                break
            }
            case "embed": {
                let caption = block.embed.caption.map(text => this.richTextToString(text)).join("")
                if (!caption) {
                    caption = "View embed"
                }
            ...
            case "bookmark": {
                let caption = block.bookmark.caption.map(text => this.richTextToString(text)).join("")
                if (!caption) {
