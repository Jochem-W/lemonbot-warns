...
export class MessageUpdateHandler implements Handler<"messageUpdate"> {
    ...
    public async handle(_oldMessage: Message | PartialMessage, newMessage: Message | PartialMessage) {
        const prismaMessage = await Prisma.message.findFirst({where: {id: newMessage.id}})
        if (!prismaMessage) {
            return
        }
